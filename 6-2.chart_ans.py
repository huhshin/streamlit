...
st.header('Unit 2. Streamlit Simple chart')
...
st.subheader('2-1. Simple Line chart')
#use_container_width=True 가로로 화면에 꽉 채워 줌
st.line_chart(chart_data, use_container_width=True) 

st.subheader('2-2. Simple Bar chart')
st.bar_chart(chart_data)

st.subheader('2-3. Simple area chart')
st.area_chart(chart_data)

st.header('Unit 3. Altair chart') 
...
st.subheader('3-1. Altair Line chart')
...
st.subheader('3-2. Altair Bar chart')
...
st.subheader('3-3. Altair Scatter chart')
...
st.subheader('4-1. Plotly Pie/Donut chart') 
fig = px.pie(medal, values = "gold", names = "nation", title="올림픽 양궁 금메달 현황") # 도넛차트: hole=.3)
fig.update_traces(textposition='inside', textinfo = 'percent+label')
fig.update_layout(font = dict(size = 16))
# fig.update(layout_showlegend=False)  # 범례 표시 제거
st.plotly_chart(fig)

st.subheader('4-2. Plotly Bar chart')
# text_auto=True 값 표시 여부
fig = px.bar(medal, x="nation", y=["gold", "silver", "bronze"],
             text_auto=True, title="올림픽 양궁 메달 현황")  
st.plotly_chart(fig)

# 파일실행: File > New > Terminal(anaconda prompt) - streamlit run streamlit\6-2.chart_ans.py